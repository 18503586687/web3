...
!devlopmentChains.includes(network.name)
  ...
  : describe("test fundme contract", async function () {
      ...
      beforeEach(async function () {
        ...
        const fundMeDeploment = await deployments.get("FundMe");
        fundMe = await ethers.getContractAt("FundMe", fundMeDeploment.address);
      ...
      it("window closed, value grater than minimum, fund failed", async function () {
        ...
        await expect(
          fundMe.fund({ value: ethers.parseEther("0.0007") })
      ...
      it("Window open, value is greater minimum, fund success", async function () {
        ...
        await fundMe.fund({ value: ethers.parseEther("0.0007") });
        const balance = await fundMe.fundersToAmount(firstAccount);
        await expect(balance).to.equal(ethers.parseEther("0.0007"));
      });
      ...
      it("window closed, target reached, getFund success", async function () {
        ...
        await expect(fundMe.getFund())
          ...
          .withArgs(ethers.parseEther("0.007"));
      });
